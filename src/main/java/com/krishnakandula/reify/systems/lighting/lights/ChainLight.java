...
public class ChainLight extends Light {
     ...
     *            </ul>
     */
    public ChainLight(int rays,
                      Color color,
                      float distance,
                      int rayDirection) {
        this(rays, color, distance, rayDirection, null);
    }
    ...
    @Override
    void update(RayHandler rayHandler) {
        if (dirty) {
            updateChain();
            applyAttachment();
        } else {
    ...
    @Override
    void render(RayHandler rayHandler) {
     ...
     * <p>Actual recalculations will be done only on {@link #update(RayHandler)} call
     */
    @Override
    ...
    protected boolean cull(RayHandler rayHandler) {
        if (!rayHandler.culling) {
            culled = false;
        } else {
            updateBoundingRects(rayHandler);
    ...
    protected void updateMesh(RayHandler rayHandler) {
        for (int i = 0; i < rayNum; i++) {
            m_index = i;
            f[i] = 1f;
            tmpEnd.x = endX[i];
    ...
    protected void updateBoundingRects(RayHandler rayHandler) {
        float maxX = startX[0];
        float minX = startX[0];
        float maxY = startY[0];
        float minY = startY[0];
